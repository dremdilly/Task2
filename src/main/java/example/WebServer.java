...
public class WebServer {
    public static void main(String[] args) {
        ...
        MultithreadQueue<Socket> queueOfThreads = new MultithreadQueue<>();
        try (ServerSocket serverSocket = new ServerSocket(port, queueLength)) {
            ...
            for(int i = 0; i < numOfThreads; i++ ) {
                var worker = new Worker(i, queueOfThreads);
                worker.start();
            }
            ...
            while (true) {
                ...
                // Add request in the shared queue
                queueOfThreads.add(socket);
            }
        }
